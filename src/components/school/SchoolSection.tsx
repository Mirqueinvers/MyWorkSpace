...
function getPrimaryLink(student: SchoolStudent) {
  return student.rmisUrl?.trim() || student.links[student.links.length - 1]?.url?.trim() || ''
}
...
async function copyTextToClipboard(value: string) {
  if (navigator.clipboard?.writeText) {
    try {
      ...
      // Fall back to the legacy path below when clipboard permissions are unavailable.
    }
  }
...
async function openStudentLink(url: string) {
  if (!url) {
    return
  }

  try {
    await window.electronAPI?.school?.openLink?.(url)
  } catch {
...
export function SchoolSection({
  ...
  async function handleCopyStudentKey(student: SchoolStudent) {
    try {
      await copyTextToClipboard(getStudentClipboardKey(student))
      setViewedStudentIds((currentValue) => {
        const nextValue =
          currentValue === VIEWED_STUDENTS_STORAGE_UNINITIALIZED ? {} : currentValue
      ...
      alert('Не удалось скопировать ключ ученика.')
    }
  }
            ...
            {selectedClass.students.length === 0 ? (
              ...
              <div className="xray-journal-list">
                {selectedClass.students.map((student) => {
                  ...
                  const primaryLink = getPrimaryLink(student)
                      ...
                      <div className="xray-journal-item-head">
                        <div className="xray-fl-journal-patient-line">
                          ...
                          <strong
                            ...
                            onClick={() => {
                              void openStudentLink(primaryLink)
                            }}
                            role={hasLink ? 'button' : undefined}
                            tabIndex={hasLink ? 0 : undefined}
                            onKeyDown={(event) => {
                              if (hasLink && (event.key === 'Enter' || event.key === ' ')) {
                          ...
                          <button
                            ...
                            onClick={() => {
                              const isOpening = !linkFormOpenByStudentId[student.id]
